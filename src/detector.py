...
from dataset.dataset_utils import affine_transform, draw_umich_gaussian, gaussian_radius, get_affine_transform
...
class Detector(object):
    def __init__(self, opt, model):
        ...
        self.opt = opt
        self.trained_dataset = get_dataset(opt.dataset,'val',opt,mini_version=True)
        self.mean = np.array(
    ...
    def run(self, image_or_path_or_tensor, meta={}):
        ...
        if self.opt.save_video:
            try:
                ...
                ret.update({k: v.copy() for k,v in self.debugger.imgs.items()})
            except:
    ...
    def _get_additional_inputs(self, dets, meta, with_hm=True):
        ...
        if with_hm:
            ...
            input_hm = tf.convert_to_tensor(input_hm)#.to(self.opt.device)
        output_inds = np.array(output_inds, np.int64).reshape(1, -1)
        output_inds = tf.convert_to_tensor(output_inds)#.to(self.opt.device)
    ...
    def _sigmoid_output(self, output):
        if 'hm' in output:
            output['hm'] = tf.math.sigmoid(output['hm'])
        if 'hm_hp' in output:
            output['hm_hp'] = tf.math.sigmoid(output['hm_hp'])
        if 'dep' in output:
            output['dep'] = 1. / (tf.math.sigmoid(output['dep']) + 1e-6) - 1.
    ...
    def process(self, images, pre_images=None, pre_hms=None,
        ...
        output = self.model(images, pre_images, pre_hms, training=False)[-1]
        ...
        if scale != 1:
            for i in range(len(dets[0])):
                for k in ['bbox', 'hps']:
                    if k in dets[0][i]:
                        dets[0][i][k] = (np.array(
                                dets[0][i][k], np.float32) / float(scale)).tolist()
        return dets[0]
